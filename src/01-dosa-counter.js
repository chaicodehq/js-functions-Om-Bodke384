...
export function calculateDosaOrder(type, quantity = 1, isSpicy = false) {
  // Your code here
  if(typeof type !=="string"|| quantity<=0 || quantity===NaN || type===null || type ==="unknown" || type===undefined) return null
  const Base_Price = {
    "plain":40,
    "masala":60,
    "onion":50,
  ...
  if(Base_Price[type]===undefined) return null
  if(typeof quantity !== "number") return null
  if(isNaN(quantity)) return null

  const pricePerDosa = parseInt(Base_Price[type]) +  parseInt((isSpicy)?10:0)
